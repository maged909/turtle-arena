...
static void CG_Item( centity_t *cent ) {
	...
	if( item->giType == IT_WEAPON ) {
		...
		if (BG_WeaponHasMelee(item->giTag)) {
			...
			Byte4Copy( pi->c2RGBA, ent.shaderRGBA );
		}
#else
		Byte4Copy( pi->c1RGBA, ent.shaderRGBA );
#endif
...
*/
	...
	if ( projectile->missileDlight ) {
		trap_R_AddLightToScene(cent->lerpOrigin, projectile->missileDlight, 1.0f,
			projectile->missileDlightColor[0], projectile->missileDlightColor[1], projectile->missileDlightColor[2], 0 );
	}
