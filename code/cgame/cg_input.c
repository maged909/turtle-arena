...
void CG_FinishMove( localPlayer_t *player, usercmd_t *cmd ) {
	...
	weaponSelect = 0;
#else
	weaponSelect = player->weaponSelect;
#endif
#ifdef TA_HOLDSYS/*2*/
	holdableSelect = player->holdableSelect;
#else
	holdableSelect = 0;
#endif
	...
	localPlayerNum = player - cg.localPlayers;
	if (cg_thirdPerson[localPlayerNum].integer && cg_thirdPersonAnalog[localPlayerNum].integer
		&& cg_thirdPersonAngle[localPlayerNum].value)
	{
		if (cmd->forwardmove || cmd->rightmove || cmd->upmove)
		{
			player->viewangles[YAW] -= cg_thirdPersonAngle[localPlayerNum].value;
			trap_Cvar_Set(Com_LocalPlayerCvarName(localPlayerNum, "cg_thirdPersonAngle"), "0");
		}
	}
#endif
	for (i=0 ; i<3 ; i++) {
...
#ifdef TA_PATHSYS // 2DMODE
	if ( cg.snap && cg.snap->playerNums[ localPlayerNum ] != -1 ) {
		ci->pathMode = cg.snap->pss[ localPlayerNum ].pathMode;
	} else {
