...
#ifdef TURTLEARENA // LOCKON
void IN_LockonDown( int localPlayerNum ) {IN_KeyDown(&cis[localPlayerNum].in_lockon);IN_KeyDown(&cis[localPlayerNum].in_strafe);}
void IN_LockonUp( int localPlayerNum ) {IN_KeyUp(&cis[localPlayerNum].in_lockon);IN_KeyUp(&cis[localPlayerNum].in_strafe);}
#endif
...
void CG_FinishMove( cglc_t *lc, usercmd_t *cmd ) {
	int i;
	int weaponSelect, holdableSelect;
	...
	weaponSelect = 0;
#else
	weaponSelect = lc->weaponSelect;
#endif
#ifdef TA_HOLDSYS/*2*/
	holdableSelect = lc->holdableSelect;
#else
	holdableSelect = 0;
#endif
	cmd->stateValue = BG_ComposeUserCmdValue( weaponSelect, holdableSelect );

#ifdef IOQ3ZTM // ANALOG
	// If cg_thirdPersonAnalog, always move relative to camera.
	localClientNum = lc - cg.localClients;
	if (cg_thirdPerson[localClientNum].integer && cg_thirdPersonAnalog[localClientNum].integer
		&& cg_thirdPersonAngle[localClientNum].value)
	{
		if (cmd->forwardmove || cmd->rightmove || cmd->upmove)
		{
			lc->viewangles[YAW] -= cg_thirdPersonAngle[localClientNum].value;
			trap_Cvar_Set(Com_LocalClientCvarName(localClientNum, "cg_thirdPersonAngle"), "0");
		}
	}
#endif
	for (i=0 ; i<3 ; i++) {
...
#ifdef TA_PATHSYS // 2DMODE
	if ( cg.snap && cg.snap->clientNums[ localClientNum ] != -1 ) {
		ci->pathMode = cg.snap->pss[ localClientNum ].pathMode;
	} else {
