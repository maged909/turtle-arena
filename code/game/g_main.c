...
static cvarTable_t		gameCvarTable[] = {
	...
	{ &g_fraglimit, "scorelimit", "1000", CVAR_SERVERINFO | CVAR_ARCHIVE | CVAR_NORESTART, GCF_TRACK_CHANGE, RANGE_INT(0, INT_MAX) },
#else
	...
	{ &g_quadfactor, "g_quadfactor", "2", 0, GCF_TRACK_CHANGE, RANGE_ALL },
#else
...
#ifdef MISSIONPACK
#ifdef TURTLEARENA
	{ &g_obeliskHealth, "g_obeliskHealth", "1000", 0, 0, RANGE_ALL },
	{ &g_obeliskRegenPeriod, "g_obeliskRegenPeriod", "1", 0, 0, RANGE_ALL },
	{ &g_obeliskRegenAmount, "g_obeliskRegenAmount", "3", 0, 0, RANGE_ALL },
#else
...
void BeginIntermission( void ) {
	...
	for (i=0 ; i< level.maxplayers ; i++) {
		...
		if ( g_gametype.integer == GT_SINGLE_PLAYER ) {
			...
			player->player->ps.pm_type = PM_SPINTERMISSION;
			continue;
		}
#endif
...
void G_RunFrame( int levelTime ) {
	...
	for (i=0 ; i<level.num_entities ; i++, ent++) {
		...
		if ( ent->s.eType == ET_GRAPPLE ) {
			if (!ent->parent || !ent->parent->inuse || !ent->parent->player)
			{
				G_FreeEntity( ent );
				continue;
			}
