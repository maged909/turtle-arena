...
void CG_SurfaceRailCore( const refEntity_t *originEnt ) {
	...
	if ( re.radius ) {
		spanWidth = (cg_railCoreWidth.value / 6.0f) * re.radius;
	} else {
		spanWidth = cg_railCoreWidth.integer;
	}

	CG_DoRailCore( verts, re.shaderRGBA, start, end, right, len, spanWidth );
#else
...
#ifdef TA_WEAPSYS
		CG_DoRailCore( &verts[i*4], re.shaderRGBA, start, end, right, len, spanWidth );
#else
